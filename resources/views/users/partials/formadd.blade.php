    ...
    <div class="col-md-4">
        ...
        <select class="form-control {{ $errors->has('id_pais') ? ' is-invalid' : '' }}" name="id_pais" id="id_pais" required value="{{ @old("id_pais", $user->id_pais) }}">
            ...
            @foreach($paises AS $pais)
                <option value="{{$pais->id_pais}}" @if($pais->id_pais==$user->id_pais) selected @endif>{{$pais->pais}}</option>
            @endforeach
        </select>
...
<div class="row mt-4">
    <div class="col-sm-6">
        ...
        <input id="password" name="password" type="password" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" @if(!$user->id_cliente) required @endif>
